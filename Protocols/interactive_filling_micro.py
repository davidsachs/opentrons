...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Collect 150 liquid
    p1000_multi.aspirate(400, tube_rack["A1"])

    # Move to "in chip" position (saved location 0)
    move_to_chip()

    p1000_multi.dispense(40, flow_rate=dispense_rate)
    for i in range(5):
        move_to_next_well(i)
    ...
    protocol.comment("Pause - warm up")
    protocol.comment("Picking up tip...")
    
    p1000_multi.pick_up_tip()
    p1000_multi.move_to(above_chip_location)
    protocol.comment("Pause - reset chip location")
    ...
    p1000_multi.prepare_to_aspirate()
    for i in range(2):
    ...
    # Return microscope to well 0 before blow out
    return_microscope_to_start()
