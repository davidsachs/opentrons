...
metadata = {
    'protocolName': 'Emptying (microscope follows)',
    'author': 'David Sachs',
    'description': 'Emptying - microscope tracks the active well in X/Y',
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    protocol.comment(f"=== Aspirate ===")
    emptying_volume = 30
    # Collect 150 liquid
    for i in range(6):
        if i!=0:
            return_to_well(i)
        p1000_multi.aspirate(emptying_volume, flow_rate=aspirate_rate)
        lift_out()
    ...
    protocol.comment("Pause - E to exit, tab to continue")#or do another seeding")
