...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Step 2: Aspirate 50uL consecutively from plate columns A1, A2, A3, A4
    protocol.comment("Step 2: Aspirating 50uL from plate columns A1-A4...")
    for col in ['A1', 'A2', 'A3', 'A4']:
        p200_multi.aspirate(
            50,
            plate_96[col].bottom(ASPIRATE_HEIGHT + PLATE_Z_OFFSET).move(
                types.Point(x=PLATE_X_OFFSET, y=0, z=0)
            ), flow_rate=aspirate_rate
        )

    # Step 3: Blow out all aspirated waste to reservoir A2
    protocol.comment("Step 3: Blow out to waste...")
    p200_multi.blow_out(waste_location)

    # Step 4: Aspirate 200uL from reservoir A1 and dispense into plate columns A1-A4
    protocol.comment("Step 4: Dispensing fresh media to plate columns A1-A4...")
    for col in ['A1', 'A2', 'A3', 'A4']:
        p200_multi.aspirate(200, media_location, flow_rate=aspirate_rate)
        p200_multi.dispense(
            200,
            plate_96[col].bottom(DISPENSE_HEIGHT + PLATE_Z_OFFSET).move(
                types.Point(x=PLATE_X_OFFSET, y=0, z=0)
            ), flow_rate=dispense_rate
        )

    # Step 5: Discard tips into trash bin at A3 and home
    protocol.comment("Step 5: Discarding tips...")
