...
metadata = {
    'protocolName': 'Empty/fill/mix (microscope follows)',
    'author': 'David Sachs',
    'description': 'Microscope tracks the active well in X/Y',
}
...
ABOVE_CHIP_X = 389.4
ABOVE_CHIP_Y = 161.7
ABOVE_CHIP_Z = 12  # High enough to be safely above the chip
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    fill_dispense_rate = 4#6
    fill_volume = 35
    mix_aspirate_rate = 5
    mix_dispense_rate = 10
    mix_volume = 20
    empty_aspirate_rate = 5
    emptying_volume = 35
    # Collect 150 liquid
    for i in range(6):
        if i!=0:
            return_to_well(i, move_microscope=True)
        protocol.comment("F0.5")
        protocol.comment("MR X0.8 Y0.8")
        protocol.comment("F0")
        p1000_multi.aspirate(emptying_volume, flow_rate=empty_aspirate_rate)
        protocol.comment("F0.5")
        protocol.comment("MR X-0.8 Y-0.8")
        protocol.comment("F0")
        lift_out()
        if i==0:
            p1000_multi.dispense(emptying_volume, tube_rack["A2"])
        else:
            p1000_multi.dispense(emptying_volume+5, tube_rack["A2"])
        if i==0:
            p1000_multi.aspirate(fill_volume, tube_rack["A1"])
            p1000_multi.dispense(fill_volume)
            p1000_multi.aspirate(fill_volume)
            p1000_multi.dispense(fill_volume)
            p1000_multi.aspirate(fill_volume)
            p1000_multi.dispense(fill_volume)
            p1000_multi.aspirate(fill_volume)
            p1000_multi.dispense(fill_volume)
            p1000_multi.aspirate(fill_volume+5)
        else:
            p1000_multi.aspirate(fill_volume+5, tube_rack["A1"])
        return_to_well(i, move_microscope=False, corner=True)
