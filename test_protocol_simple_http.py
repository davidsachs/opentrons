...
Original API Level: 2.19
Robot Type: OT-2 Standard
"""
...
def execute_protocol(runner: HTTPProtocolRunner) -> None:
    """Execute the protocol commands."""
    ...
    # pipette.pick_up_tip()
    runner.pick_up_tip("pipette", "tip_rack", "A1")

    # pipette.aspirate(100, plate["A1"])
    runner.execute_command("aspirateInPlace", {
        "volume": 100,
        "pipetteId": runner.pipette_ids[\"pipette\"]
})

    # pipette.dispense(100, plate["B1"])
    runner.execute_command("dispenseInPlace", {
        "volume": 100,
        "pipetteId": runner.pipette_ids[\"pipette\"]
})

    # pipette.drop_tip()
    runner.drop_tip("pipette")
